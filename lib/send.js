...
var test = require("rc-mobile-base/lib/selectors/rooms")
...
var send = function send(action, dispatch, config, retries, isOfflineToOnline) {
  var retries = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 0;
  var metadata = action.meta.offline;
  var checkOfflineToOnline = isOfflineToOnline()
  dispatch((0, _actions.busy)(true));
  if ((action.type === "OUTBOUND_ROOM_MODEL_UPDATE" || action.type === "OUTBOUND_TASK_CREATE" || action.type === "OUTBOUND_TASK_REASSIGN" || action.type === "OUTBOUND_TASK_UPDATE")  && checkOfflineToOnline) {
    setTimeout(() => {
  ...
  }else if(action.type === "OUTBOUND_TASK_CREATE" || action.type === "OUTBOUND_TASK_REASSIGN" || action.type === 'OUTBOUND_TASK_UPDATE'){
    setTimeout(() => {
      return config.effect(metadata.effect, action).then(function (result) {
        var commitAction = metadata.commit || _objectSpread(_objectSpread({}, config.defaultCommit), {}, {
          meta: _objectSpread(_objectSpread({}, config.defaultCommit.meta), {}, {
