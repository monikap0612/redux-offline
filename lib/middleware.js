    ...
    return function (next) {
      return function (action) {
        ...
        if (offlineAction && !offline.busy && !offline.retryScheduled && offline.online) {
          const isOfflineToOnline = () => {
            return state.rooms.isOfflineToOnline;
        };
          (0, _send.default)(offlineAction, store.dispatch, config, offline.retryCount, isOfflineToOnline);
        }
        ...
        if (action.type === _constants.OFFLINE_SEND && offlineAction && !offline.busy) {
          (0, _send.default)(offlineAction, store.dispatch, config, offline.retryCount, false);
        }
